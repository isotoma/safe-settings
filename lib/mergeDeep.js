...
class MergeDeep {
  ...
  processArrays (key, source, target, deletions, additions, modifications) {
    ...
    if (combined.length > 0) {
      // Elements that not in source are deletions
      const removed = combined.filter(item => {
        if (this.isObjectNotArray(item)) {
          return !source.some(sourceItem => GET_NAME_USERNAME_PROPERTY(item) === GET_NAME_USERNAME_PROPERTY(sourceItem))
        } else {
          return !source.includes(item)
      ...
      // Entries present in BOTH arrays have already had their own nested
      // deletions collected into deletions[key] by compareDeepIfVisited - e.g. a
      // rule removed from a ruleset that still exists. Assigning over the top
      // discarded those, so a dry-run under-reported: `changed()` saw the
      // deletion and issued the update, but the summary showed nothing.
      //
      // Keep those sub-diffs and append the whole-element removals. The two sets
      // are disjoint: a sub-diff describes an element that is still in source,
      // whereas a removal is by definition absent from source. Filtering on that
      // also avoids double-counting the `dels` computed above, which covers the
      // same elements as `removed`.
      const nested = Array.isArray(deletions[key])
        ? deletions[key].filter(d => {
          if (this.isEmpty(d)) return false
          const id = GET_NAME_USERNAME_PROPERTY(d)
          return id !== undefined && source.some(item => GET_NAME_USERNAME_PROPERTY(item) === id)
        })
        : []
      deletions[key] = nested.concat(removed)
    }
  }
  ...
  compareDeepIfVisited (additions, modifications, deletions, a, visited) {
    ...
    if (visited[id]) {
      ...
      // Add name attribute to deletions for the same reason
      if (!this.isEmpty(deletions[deletions.length - 1])) {
        if (visited[id]) {
          deletions[deletions.length - 1][NAME_USERNAME_PROPERTY(a)] = id
        }
      }
      if (visited[id]) {
        delete visited[id]
