      ...
      return this.find().then(existingRecords => {
        ...
        if (!compare.hasChanges) {
          ...
          if (this.nop) {
            ...
            this.log.info(`[${this.constructor.name}] ${this.repo.owner}/${this.repo.repo}: applying changes`)
          }
        }

        const itemName = item => MergeDeep.NAME_FIELDS.reduce((n, f) => n || item[f], null)
        ...
        existingRecords.forEach(x => {
          if (!filteredEntries.find(y => this.comparator(x, y))) {
            if (!this.nop) this.log.info(`[${this.constructor.name}] ${this.repo.repo}: removing ${itemName(x) || JSON.stringify(x)}`)
        ...
        filteredEntries.forEach(attrs => {
          ...
          if (!existing) {
            if (!this.nop) this.log.info(`[${this.constructor.name}] ${this.repo.repo}: adding ${itemName(attrs) || JSON.stringify(attrs)}`)
            ...
            if (!this.nop) this.log.info(`[${this.constructor.name}] ${this.repo.repo}: updating ${itemName(attrs) || JSON.stringify(attrs)}`)
