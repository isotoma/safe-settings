          ...
          .map(branch => {
            ...
            if (this.isEmpty(branch.protection)) {
              ...
              return this.github.rest.repos.getBranchProtection(params).then((result) => {
                ...
                return this.github.rest.repos.updateBranchProtection(params).then(res => this.log.info(`[Branches] ${this.repo.owner}/${this.repo.repo}: branch protection applied for ${params.branch}`)).catch(e => { this.logError(`Error applying branch protection ${JSON.stringify(e)}`); return [] })
              }).catch((e) => {
                if (e.status === 404) {
