...
module.exports = class Branches extends ErrorStash {
          ...
          .map(branch => {
            ...
            if (this.isEmpty(branch.protection)) {
              ...
              return this.github.repos.getBranchProtection(params).then((result) => {
                ...
                this.log.info(`[Branches] ${this.repo.owner}/${this.repo.repo}: applying branch protection for ${params.branch}`)
                return this.github.repos.updateBranchProtection(params).then(res => this.log.info(`[Branches] ${this.repo.owner}/${this.repo.repo}: branch protection applied for ${params.branch}`)).catch(e => { this.logError(`Error applying branch protection ${JSON.stringify(e)}`); return [] })
              }).catch((e) => {
                if (e.status === 404) {
  ...
  reformatAndReturnBranchProtection (protection) {
    if (protection) {
      ...
      protection.allow_deletions = protection.allow_deletions && protection.allow_deletions.enabled
      ...
      if (protection.required_pull_request_reviews) {
        const bpa = protection.required_pull_request_reviews.bypass_pull_request_allowances
        if (!bpa) {
          protection.required_pull_request_reviews.bypass_pull_request_allowances = { apps: [], teams: [], users: [] }
        } else {
          if (Array.isArray(bpa.teams)) {
            bpa.teams = bpa.teams.map(t => (typeof t === 'object' ? t.slug : t))
          }
          if (Array.isArray(bpa.users)) {
            bpa.users = bpa.users.map(u => (typeof u === 'object' ? u.login : u))
          }
          if (Array.isArray(bpa.apps)) {
            bpa.apps = bpa.apps.map(a => (typeof a === 'object' ? a.slug : a))
          }
        }
      }
    }
