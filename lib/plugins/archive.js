...
module.exports = class Archive {
  ...
  async updateRepoArchiveStatus (archived) {
    ...
    if (this.nop) {
      const change = { msg: `Repository will be ${action}d`, additions: null, modifications: { archived }, deletions: null }
      return new NopCommand(
        this.constructor.name,
        this.repo,
        this.github.rest.repos.update.endpoint({ owner: this.repo.owner, repo: this.repo.repo, archived }),
        change,
        'INFO'
      )
    }

    await this.github.rest.repos.update({
      owner: this.repo.owner,
      repo: this.repo.repo,
      archived
    })

    this.log.info(`[Archive] ${this.repo.owner}/${this.repo.repo}: ${action}d`)
  }
