...
`
  ...
  async checkAndProcessRepo (owner, name) {
    if (this.isRestricted(name)) {
      // this.log.info(`Skipping repo: ${owner}/${name}`)
      return null
    }
    this.log.info(`Processing repo: ${owner}/${name}`)
  ...
  async loadConfigMap (params) {
    if (env.LOCAL_CONFIG_PATH) {
      const localPath = path.join(env.LOCAL_CONFIG_PATH, params.path)
      try {
        ...
        if (stat.isDirectory()) {
          ...
          return files
            ...
            .map(f => ({ name: f, path: path.posix.join(params.path, f) }))
        }
  ...
  async getRepoConfigMap () {
    if (env.LOCAL_CONFIG_PATH) {
      const reposDir = path.join(env.LOCAL_CONFIG_PATH, CONFIG_PATH, 'repos')
      try {
        const files = fs.readdirSync(reposDir)
        return files
          .filter(f => f.endsWith('.yml') || f.endsWith('.yaml'))
          .map(f => ({ name: f, path: path.posix.join(CONFIG_PATH, 'repos', f) }))
      } catch (e) {
        if (e.code === 'ENOENT') return []
        throw e
      }
  ...
  async loadYaml (filePath) {
    if (env.LOCAL_CONFIG_PATH) {
      const localPath = path.join(env.LOCAL_CONFIG_PATH, filePath)
      try {
        return yaml.load(fs.readFileSync(localPath, 'utf8')) || {}
      } catch (e) {
